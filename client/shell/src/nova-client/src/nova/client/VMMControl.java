...
public class VMMControl implements IVMMControl {
	...
	public List<IVM_INFO> getVirtualClusterInfo(VM_HANDLE machine) {
		List<IVM_INFO> list = new ArrayList<IVM_INFO>();
...
//				ivi.put(VM_INFO.MIGRATION_TARGET, vmachineinfo.get("migrate_to").getAsString());
	...
	public void startVirtualMachine(VM_HANDLE machine, String sched_to, IVMPowerCallback callback) {
			new VMOperateThread(machine, sched_to, callback, OperationOnVM.START).start();
			System.out.println("start Virtualmachine with sched_to finished successfully(VMMControl)");
	}
