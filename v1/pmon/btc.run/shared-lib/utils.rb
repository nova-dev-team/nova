...
EOF
  ...
  # dbus 方法调用。
  #  call_bus sender { |bus| your code here }
  #  sender 长的像 cn.org.btc.StorageEngine 这样
  def call_bus(sender, type='system')
    _s = sender.split '/'
    _s.delete_at(0) if _s[0] == ""
    _busname = _s.join('.')
    _interface = _s.join('.')
    ...
    conn = bus.service(_busname)
    obj = conn.object(sender)
    obj.default_iface = _interface
    obj.introspect
