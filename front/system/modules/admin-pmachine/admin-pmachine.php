...
class AdminPmachine
{
    ...
    public function changePMstatus() {
      $core_reply = "";
      if ($_REQUEST["is_working"] == "true") {
        $core_reply = file_get_contents("http://localhost:3000/pmachine/unmark_remove/" . $_REQUEST["pm_ip"]);
      } else {
        $core_reply = file_get_contents("http://localhost:3000/pmachine/mark_remove/" . $_REQUEST["pm_ip"]);
      }
      echo $core_reply;
    }
