...
xsuccess xlog_init(int argc, char* argv[]) {
  ...
  if (xoption_has(xopt, "log-level")) {
    ...
    xlog_ctl("default-console", XLOG_CTL_SET_LEVELS, log_level_flag);
    xlog_ctl("default-file", XLOG_CTL_SET_LEVELS, log_level_flag);
  }
  ...
  if (xoption_has(xopt, "log-maxsize")) {
    ...
    xlog_ctl("default-file", XLOG_CTL_SET_SIZE, log_max_size);
  }

  if (xoption_has(xopt, "log-history")) {
    int log_history = atoi(xoption_get(xopt, "log-history"));
    xlog_ctl("default-file", XLOG_CTL_SET_HISTORY, log_history);
  }

  if (xoption_has(xopt, "log-screenonly")) {
    xlog_remove("default-file");
  } else {
    xlog_ctl("default-file", XLOG_CTL_SET_FILE, xstr_get_cstr(log_fn));
  }
...
static xsuccess xlog_ctl_add(const char* logger_name) {
  ...
  if (ret == XSUCCESS) {
    // by default, the logger file is XLOG_DEFAULT_FILE
    lg = xmalloc_ty(1, xlogger_ty);
    lg->name = strdup(logger_name);
    lg->pattern = strdup(DEFAULT_XLOGGER_PATTERN);
    lg->fn = NULL;  // no file
    lg->fp = XLOG_DEFAULT_FILE;
    lg->enabled = XTRUE;
    lg->levels = 0x7; // fatal,debug,warning
    lg->special = XTRUE;
...
xsuccess xlog_ctl(const char* logger_name, xlog_ctl_action action, ...) {
  ...
  default:
    ...
    fprintf(XLOG_DEFAULT_FILE, "xlog_clt action not understood! action id = %d\n", action);
    ret = XFAILURE;
    break;
  }
...
static xsuccess rotate_log(xlogger_ty* lg) {
  ...
  if (i <= lg->history) {
    xstr history_log_fn = xstr_new();
      printf("HERE %s %s\n", lg->fn, xstr_get_cstr(current_log_fn));
...
static xbool do_log_visitor(const void* key, void* value, void* args) {
  ...
  if (lg->enabled == XTRUE && ((lg->levels >> level) & 0x1) == 1) {
    char* log_level_names[] = {"fatal", "error", "warning", "info", "debug", "level5", "level6", "level7"};
    char* p = lg->pattern;
    char timestr_buf[32];
    struct tm* tm_struct;
