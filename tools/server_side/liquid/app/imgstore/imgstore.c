...
static void cmd_acceptor(xsocket client_xs, void* args) {
  ...
  xlog_info("[imgstore] client connected\n");

  xfree(inbuf);
  xfree(outbuf);
}

static xsuccess imgstore_service(xstr host, int port, int backlog) {
  xsuccess ret;
  void* args = host;
  ...
  if (xs == NULL) {
    xlog_fatal("[imgstore] in imgstore_service(): failed to init xserver!\n");
    ret = XFAILURE;
  } else {
    xlog_info("[imgstore] imgstore server started on %s:%d\n", xstr_get_cstr(host), port);
...
static xsuccess imgstore_real(int argc, char* argv[]) {
  ...
  xoption_delete(xopt);
  ret = imgstore_service(bind_addr, port, back_log);
  return ret;
}
...
xsuccess liquid_imgstore(int argc, char* argv[]) {
  ...
  if (xoption_has(xopt, "help") || xoption_has(xopt, "h")) {
    liquid_imgstore_help();
  } else {
    // check if keepalive is required
    if (xoption_has(xopt, "keepalive")) {
      ret = xkeep_alive(imgstore_real, argc, argv);
    } else {
      ret = imgstore_real(argc, argv);
    }
  }
